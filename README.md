...
|----------|------------------------|----------|
| **Языки (languages)** | *пусто* | Список языков через запятую, например: `en,ru,ua`. Если оставить пустым, используются все активные языки из конфигурации Cotonti (`Cot::$cfg['languages']`). |
